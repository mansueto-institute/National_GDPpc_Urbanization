...
for row in readerfff:
    if (row[1]!=[] and row[0].isdigit() and int(row[0])>10000):
        ...
        if (gamma_av>4.5 or gamma_std > 7.):
...
plt.plot((mqq,mqq),(0.0,15),'r--')
plt.plot((md,md),(0.0,15),'r--')
plt.plot((Mqq,Mqq),(0.0,15),'r--')


plt.ylabel('$\sigma_{\gamma_i}$',fontsize=20)
plt.xlabel('$\log_{10} \ N_i$',fontsize=20)
plt.tight_layout()
#plt.show()
plt.savefig('GDPpc_growth_rates_std.pdf', format='pdf')
