    ...
    if (row[1]!='' and row[3]!='' and row[4]!='' and row[4].isdigit() and float(row[3])<99. ):
        ...
        if (nation!=row[0]):
            ...
            if (cnt>0 and nation=='United States'):#  or  nation=='South Korea'): # and len(ur)>60):
                ...
                plt.plot(xx,yy,marker='o',ms=6,ls='None',c='blue',markeredgecolor=edge_color,markeredgewidth=1,alpha=0.8,label=str(nation))
...
#plt.legend()
#plt.show()
plt.savefig('Trajectories_United_States_OWID.pdf', format='pdf')
